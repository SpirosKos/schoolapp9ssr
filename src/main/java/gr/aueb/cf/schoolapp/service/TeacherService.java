...
//    }
    ...
    public TeacherReadOnlyDTO saveTeacher(TeacherInsertDTO dto)
        ...
        try {
            if (dto.vat() != null && teacherRepository.findByVat(dto.vat()).isPresent()) {
                throw new EntityAlreadyExistsException("Teacher with var=" + dto.vat() + " already exists");
            }
            ...
            log.error("Save failed for teacher with var={}. Teacher already exists", dto.vat());     // Structured Logging
            throw e;
    ...
    public Page<TeacherReadOnlyDTO> getPaginatedTeachers(Pageable pageable) {
        ...
        log.debug("Get paginated returned successfully page={} and size={}", teacherPage.getTotalElements(), teacherPage.getSize());
